...
app.get('/penguins/', async(req, res) => {
  try {
    const data = await client.query(`SELECT penguins.id, run testspenguins.name, penguins.number_of_feet, penguins.eats_fish, sizes.size as sizes
      from penguins
      join sizes
...
app.post('/penguins/', async(req, res) => {
  try {
    ...
    const newNumberOfFeet = req.body.number_of_feet;
    const newEatsFish = req.body.eats_fish;
    const newSize = req.body.size_id;
    const newOwnerId = req.body.owner_id;
