...
const penguins = require('../data/penguins.js');
...
app.get('/penguins/', async(req, res) => {
  try {
    const data = await client.query('SELECT penguins.name, number_of_feet, eats_fish, sizes.size
      from penguins
      join sizes
...
app.get('/penguins/:id', async(req, res) => {
  try {
    ...
    const data = await client.query('SELECT * from penguins WHERE penguins.id-$1', [penguin_id]);
...
app.post('/penguins/', async(req, res) => {
  try {
    ...
    const numberOfFeet = req.body.number_of_feet;
    const eatsFish = req.body.eats_fish;
    ...
    [name, numberOfFeet, eatsFish, size, newOwnerId]);
